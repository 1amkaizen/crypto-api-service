        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(BASE_URL, params=params) as resp:
                    if resp.status != 200:
                        logger.error(f"❌ Gagal fetch harga {token}, status: {resp.status}")
                        raise Exception("Status != 200")
                    data = await resp.json()
                    price_info = data.get(coin_id)
                    if not price_info:
                        logger.error(f"❌ Token {token.upper()} tidak ada di respons API")
                    ...
                    logger.info(f"💲 Harga real-time {token.upper()} : {price_idr:,.0f} IDR | {price_usd:.2f} USD | Kurs IDR/USD: {kurs:,.2f}")
                    return price_idr
        except Exception as e:
            logger.warning(f"⚠️ Attempt {attempt} gagal untuk {token}, retry in {delay}s")
