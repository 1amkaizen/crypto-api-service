...
# ================== LOGGING ==================
logger = logging.getLogger("monitor.ethereum_usdc")
...
USDC_CONTRACT = Web3.to_checksum_address(os.getenv("ETH_USDC_ADDRESS"))

notifier = JualNotifier()


class EthereumUSDCMonitor:
    def __init__(self):
    ...
    async def handle_tx(self, tx_hash: str, amount: float, sender: str, receiver: str):
        ...
        usdc_price_idr = await get_current_price("usd-coin")
...
# ================== ENTRY POINT ==================
async def run_usdc_monitor(order_id: str, chain: str = "eth"):
    monitor = EthereumUSDCMonitor()
