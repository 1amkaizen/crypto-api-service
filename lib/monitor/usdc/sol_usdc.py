# 📍 lib/monitor/usdc/sol_usdc.py (update WS langsung ke admin wallet)
import os
import asyncio
import logging
...
from lib.flip_disburse import disburse
from lib.coingecko import get_current_price

# ================== LOGGING ==================
logger = logging.getLogger("monitor.sol_usdc")
...
USDC_MINT = os.getenv("SOL_USDC_ADDRESS")
DECIMALS = 10**6  # USDC 6 decimals

notifier = JualNotifier()


class SolUSDCMonitor:
    def __init__(self):
    ...
    async def send_usdc(self, destination_wallet: str, amount: float):
        try:
            token = AsyncToken(
                self.client,
            ...
            logger.info(f"✅ Kirim {amount} USDC ke {destination_wallet} berhasil | txid: {tx_sig}")
            return tx_sig
        except Exception as e:
            logger.error(f"❌ Gagal kirim USDC: {e}")
            return None
    ...
    async def handle_tx(self, tx_sig: str, amount: float, sender: str, receiver: str):
        ...
        usdc_price_idr = await get_current_price("usdc")
        logger.info(f"💲 Harga USDC real-time: {usdc_price_idr} IDR")
    ...
    async def subscribe_pending_txs(self):
        logger.info(f"📌 Mulai monitor Solana USDC (SPL) di wallet admin {self.wallet_admin}")
        ...
        try:
            ...
            logger.info(f"🎯 ATA USDC Admin ditemukan: {admin_token_account}")
        except Exception as e:
            logger.critical(f"❌ Gagal hitung ATA USDC admin: {e}")
            return
        ...
        while True:
            try:
                async with websockets.connect(SOLANA_NODE) as ws:
                    ...
                    logger.info(f"🔌 WS connected → listening for incoming USDC transfers ke {admin_token_account}")
                    backoff = 1
                            ...
                            if "params" in msg and "result" in msg["params"]:
                                ...
                                if sigs.value:
                                    ...
                                    if sig not in processed_sigs:
                                        ...
                                        logger.info(f"🔹 Detected incoming USDC transfer, tx: {sig}")
                                            ...
                                            try:
                                                ...
                                                for ix in instructions:
                                                    ...
                                                    if dest and dest.lower() == admin_token_account.lower():
                                                        logger.info(f"✅ Transfer {amount} USDC dari {sender} → {dest}")
...
# ================== ENTRY POINT ==================
async def run_usdc_monitor(order_id: str, chain: str = "sol"):
    monitor = SolUSDCMonitor()
