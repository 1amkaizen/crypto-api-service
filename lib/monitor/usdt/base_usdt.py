...
# ================== LOGGING ==================
logger = logging.getLogger("monitor.base_usdt")
...
USDT_CONTRACT = Web3.to_checksum_address(os.getenv("BASE_USDT_ADDRESS"))

notifier = JualNotifier()


class BaseUSDTMonitor:
    def __init__(self):
        ...
        # Kontrak token USDT (ERC20 di Base)
    ...
    async def subscribe_pending_txs(self):
        ...
        logger.info(f"📌 Mulai monitor Base dari block {last_block}")
...
# ================== ENTRY POINT ==================
async def run_usdt_monitor(order_id: str, chain: str = "base"):
    monitor = BaseUSDTMonitor()
