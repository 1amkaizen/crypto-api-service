...
# ================== LOGGING ==================
logger = logging.getLogger("monitor.ethereum_usdt")
...
USDT_CONTRACT = Web3.to_checksum_address(os.getenv("ETH_USDT_ADDRESS"))

notifier = JualNotifier()


class EthereumUSDTMonitor:
    def __init__(self):
    ...
    async def handle_tx(self, tx_hash: str, amount: float, sender: str, receiver: str):
        ...
        logger.info(f"💲 Harga ETH real-time: {eth_price_idr} IDR")
...
# ================== ENTRY POINT ==================
async def run_usdt_monitor(order_id: str, chain: str = "eth"):
    monitor = EthereumUSDTMonitor()
